  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: MultiBlocProvider(
        providers: [
          /*BlocProvider(
            create: (_) => EventsBloc(repo)..add(EventsFetched()),
          ),*/
          BlocProvider(
            create: (_) => CategoriesBloc(EventRepositoryImpl(EventsJsonManager()))..add(FetchCategories()),
          ),
          BlocProvider(
            create: (_) => EventsBloc(EventRepositoryImpl(EventsJsonManager()))..add(FetchEvents(1)),
          ),
          BlocProvider(
